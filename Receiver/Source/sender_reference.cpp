...
private:
    ...
    Array<float> carrier_wave_A;
    Array<float> carrier_wave_B;
...
void Sender::GenerateCarrierWave() {
    ...
    {
        carrier_wave_A.add(carrier_amp * cos(j * 2 * PI * ((float)carrier_freq_A / (float)sample_rate) + carrier_phase));
        carrier_wave_B.add(carrier_amp * cos(j * 2 * PI * ((float)carrier_freq_B / (float)sample_rate) + carrier_phase));
    }
}
...
void Sender::Modulation(int* frame_bit) {
    for (int i = 0; i < num_bits_per_frame/2 * num_samples_per_bit; i++)
        frame_wave[i] = 0;
    for (int i = 0; i < num_bits_per_frame/2; i++) {
        for (int j = 0; j < num_samples_per_bit; j++) {
            frame_wave[i * num_samples_per_bit + j] = (frame_bit[2 * i] * 2 - 1) * carrier_wave_A[j];
            frame_wave[i * num_samples_per_bit + j] += (frame_bit[2 * i + 1] * 2 - 1) * carrier_wave_B[j];
        }
    }
}
...
void Sender::send() {
    ...
    int len_frame = header_len + num_samples_per_bit * num_bits_per_frame/2 + len_zeros;
    int len_buffer = num_frame*len_frame;
    output_buffer.setSize(1, len_buffer + 480 + len_warm_up);
    output_buffer.clear();
    for (int j = 0; j < 480; j++)
        output_buffer.setSample(0, j, carrier_wave_A[j % num_samples_per_bit]);
    for (int i = 0; i < num_frame; i++) {
        Modulation(frame_bit[i]);
        for (int j = 0; j < header_len; j++)
            output_buffer.setSample(0, len_warm_up + i * len_frame + j, header[j]);
        for (int j = 0; j < num_samples_per_bit * num_bits_per_frame/2; j++)
            output_buffer.setSample(0, len_warm_up + i * len_frame + header_len + j, frame_wave[j]);
        for (int j = 0; j < len_zeros; j++)
            output_buffer.setSample(0, len_warm_up + i * len_frame + header_len + num_samples_per_bit * num_bits_per_frame/2 + j, 0);
    }
    //ofstream of;
