...
#define USE_NRZ 0          // Non-Return to Zero: Simple baseband
#define USE_MANCHESTER 1   // Manchester: Self-clocking, DC-balanced
...
namespace ASK {
    ...
    constexpr double chirp_f_end = 6000.0;     // Was 10000.0
    ...
    void demodulate(const juce::AudioBuffer<float>& signal, const std::string& nccLogPath) {
        ...
        for (int i = 0; i < sigLen; ++i) {
            ...
            if (state == 0) {
                ...
                if (syncPower > 0.2 && syncPower > syncPowerLocalMax && syncPower > power / 5.0 ) { // 0  &&  
