...
public:
    ...
    void analyzeRecording(const juce::AudioBuffer<float>& recordedAudio)
    {
        ...
        if (frameStartSample + FSK::totalFrameDataSamples <= recordedAudio.getNumSamples()) {
            ...
            std::cerr << "Not enough samples in recording for forced demodulation." << std::endl;
        }

#else // Normal preamble detection
        ...
        // CORRECTED: Higher threshold based on your pure_chirp.png plot
        ...
        for (int i = 0; i < recordedAudio.getNumSamples(); ++i)
        {
            ...
            if (ncc > syncPowerLocalMax && ncc > NCC_DETECTION_THRESHOLD) {
                ...
                // Let's use the automatic fine-tuning you added
                ...
                // We only care about the first valid frame
