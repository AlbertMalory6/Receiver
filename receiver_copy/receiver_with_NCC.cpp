    ...
    void analyzeRecording(const juce::AudioBuffer<float>& recordedAudio)
    {
        ...
        for (int i = 0; i < recordedAudio.getNumSamples(); ++i)
        {
            ...
            if (ncc > syncPowerLocalMax && ncc > NCC_DETECTION_THRESHOLD) {
                ...
                int frameStartSample = peakSampleIndex + 1;
            ...
            if (samplesProcessedInWindow >= samplesPerWindow) {
                std::cout << "Peak NCC in " << std::fixed << std::setprecision(1) << currentTime << "-" << (currentTime + 0.5)
                    << "s: " << std::setprecision(4) << peakNCCInWindow << std::endl;
                peakNCCInWindow = 0.0;
